...
class ForgotPasswordTests(TestCase):
    ...
    def test_forgot_password_sends_email_for_existing_user(self):
        """Test password reset shows simulated email for existing user"""
        response = self.client.post(reverse('forgot_password'), {
            'email': self.user.email
        })

        self.assertEqual(response.status_code, 200)
        # Check simulated email is displayed
        self.assertContains(response, 'Simulated Email (College Project Demo)')
        self.assertContains(response, 'Password Reset - Language Learning Platform')
        self.assertContains(response, self.user.email)
        self.assertContains(response, 'reset-password')
        self.assertContains(response, 'Click the link above to reset your password')

    def test_forgot_password_no_email_for_nonexistent_user(self):
        """Test no simulated email for non-existent user but same message shown"""
        response = self.client.post(reverse('forgot_password'), {
        ...
        # But no simulated email should be displayed
        self.assertNotContains(response, 'Simulated Email (College Project Demo)')
...
class ForgotUsernameTests(TestCase):
    ...
    def test_forgot_username_sends_email_for_existing_user(self):
        """Test username reminder shows simulated email for existing user"""
        ...
        # Check simulated email is displayed
        self.assertContains(response, 'Simulated Email (College Project Demo)')
        self.assertContains(response, 'Username Reminder - Language Learning Platform')
        self.assertContains(response, self.user.email)
        self.assertContains(response, self.user.username)
        self.assertContains(response, 'Use the username and link above to log in')
...
class AccountManagementURLTests(TestCase):
    ...
    def test_forgot_username_url_resolves(self):
        ...
        from .views import forgot_username_view

...
# Import curriculum helpers used by lesson views
from .curriculum_views import _filter_lessons_by_user_level
...
def _find_user_by_username_or_email(request, username_or_email):
    ...
    try:
        ...
        try:
            ...
            # Security: Log IP only to prevent user enumeration
            logger.warning(
                'Failed login attempt - user not found from IP: %s',
                get_client_ip(request)
            )
            messages.error(request, 'Invalid username/email or password.')
            return None
...
def chatbot_query(request):
    ...
    try:
        ...
        # Validate result structure to prevent exposing sensitive data
        if not isinstance(result, dict) or 'response' not in result:
            logger.error('Chatbot API returned invalid response structure')
            return JsonResponse(
        ...
        # Catch all exceptions to prevent stack trace exposure
        logger.error('Chatbot API error: %s', type(e).__name__, exc_info=True)
